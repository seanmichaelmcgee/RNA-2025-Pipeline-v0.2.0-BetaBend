...
class EmbeddingModule(nn.Module):
    ...
    def forward(
        ...
        sequence_int = batch["sequence_int"]  # (batch_size, seq_len)
        ...
        residue_features = torch.cat(residue_features_list, dim=-1)  # (batch_size, seq_len, residue_in_dim)

        # Project to residue embedding dimension
        residue_repr = self.residue_projection(residue_features)  # (batch_size, seq_len, residue_dim)
        ...
        pair_features_list: List[torch.Tensor] = [
            pairing_probs.unsqueeze(-1),  # (batch_size, seq_len, seq_len, 1)
            coupling_matrix.unsqueeze(-1),  # (batch_size, seq_len, seq_len, 1)
        ...
        pair_repr = self.pair_projection(pair_features)  # (batch_size, seq_len, seq_len, pair_dim)
