"""
Fixed data loading functions to handle inconsistent feature file formats.
This module addresses the issue with dihedral feature files having different key structures.
"""
        ...
        try:
            with np.load(dihedral_path) as data:
                ...
                if "features" in data:
                    ...
                    # Alternate format with numbered structures
                    features["dihedral"] = {"features": data["struct_1_features"].astype(np.float32)}
                else:
                    # Unknown format - create a warning and use None
...
def patch_data_loading():
    """Patch the data_loading module with our fixed function."""
    ...
    return original_load_precomputed_features

def test_loading():
    """Test the fixed function on problematic data."""
    target_id = "R1107"
    features_dir = os.path.join(project_root, "data/processed")
    temporal_cutoff = "2022-05-01"
    
    print(f"Testing fixed loader with {target_id}")
    features = fixed_load_precomputed_features(target_id, features_dir, temporal_cutoff)
    
    # Check what we got
    print("Features loaded successfully!")
    print(f"Dihedral features: {'Available' if features['dihedral'] is not None else 'Not available'}")
    print(f"Thermo features: {'Available' if features['thermo'] is not None else 'Not available'}")
    print(f"Evolutionary features: {'Available' if features['evolutionary'] is not None else 'Not available'}")
